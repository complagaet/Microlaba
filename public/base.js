let base = () => {	
	let onload = 1, lessonName = "menu", lessonLocation = [lessonName, ""], lessList = ["voda", "classic/ice", "classic/eds"]
	let userdata = {
		profile: {
			username: "Имя",
			ava: "ava_1",
	...
	let init = () => {
			...
			document.getElementById(target + prip).oninput = () => {
				...
				if (inp.value.trim() != '') {
					if (inp.value.length <= 20) {
						document.getElementById("input_name_label" + prip).innerHTML = "Имя"
					...
					if (inp.value.length == 20) {
						document.getElementById("input_name_label" + prip).innerHTML = "Имя <span style='color: red'>(Не более 20 символов)</span>"
						nameOK = 1
					}
				}
				else {
					document.getElementById("input_name_label" + prip).innerHTML = "Имя <span style='color: red'>(Пусто)</span>"
		...
		document.getElementById("button_settings").onclick = () => {
			...
			sbros_counter = 0;
			document.getElementById("button_edit_sbros").innerHTML = "Сброс!"
		}
		...
		document.getElementById("button_edit_sbros").onclick = () => {
			sbros_counter += 1
			if (sbros_counter == 1) {
				document.getElementById("button_edit_sbros").innerHTML = "Вы уверены?"
			}
	...
	let interface = {
		...
		lessonStartButtons: (target, progress, progressMax) => {
			try {
				...
				if (progress == 0) { msg = "Начать" }
				else if (progress == progressMax) { msg = "Повторить" }
				else if (progress != 0) { msg = "Продолжить" }
		...
		menuEntryAdaptive: () => {
			...
			if (menuEntry[0].offsetWidth > 580) {
				document.getElementById("led_entry_fg").style.backgroundPosition = "left"
				document.getElementById("led_entry_fg").style.backgroundPositionX = "150px"
			}
			else {
				document.getElementById("led_entry_fg").style.backgroundPosition = "center"
				document.getElementById("led_entry_fg").style.backgroundPositionX = ""
			}
		},
		loadscreen(state) {

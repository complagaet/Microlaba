	...
	let interface = {
			...
			try {
				...
				<div class="menu_entry_mini_inside_window bobatron">
					...
					<div class="flex flex_gap_10" style="width: 100%; justify-content: space-between">
						...
						<button class="button_lesson_start bobatron" Bt-CM="0.7" id="${target}_start">Ошибка!</button>
					</div>
				</div>`
		...
		lessonStartButtons: (target, open, progress, progressMax) => {
			try {
				...
				if (open == 1) {
					if (progress == 0) { msg = `Начать${arrow}` }
					else if (progress == progressMax) { msg = `Повторить${arrow}` }
					else if (progress != 0) { msg = `Продолжить${arrow}` }
				}
				else {
					msg = `Недоступно` 

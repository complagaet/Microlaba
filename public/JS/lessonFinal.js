lessonName = `${lessonName}/final`, lessonNameL = lessonName.split("/"),
...
let main = () => {
	...
	let testHandler = {
		...
		testHeader: (num) => {
			...
			header.innerHTML = `
			<h4>Вопрос ${num + 1}</h4>
			<h4>Выполнено ${testHandler.lastCount()}/${tasks[lessonNameL[1]].amount}</h4>`
		},
		testRender: (num) => {
			...
			<button id="test_check_button" class="bobatron" Bt-CM="0.7" style="opacity: 0; pointer-events: none">Проверить</button>
			`
	...
	let interface = {
		lessonComplete: () => {
			...
			if (lessList[lessList.indexOf(lessonNameL[1]) + 1]) {
				...
				`
				userdata.lessons[lessonNameL[0]].main.grade = Math.round((userdata.lessons[lessonNameL[0]].test.grade + userdata.lessons[lessonNameL[0]].final.grade) / 2)
				userdataControl.write()
			}
